...
# 2. Declare eth0 IPv4-only, deriving address/gateway/DNS from the live system.
PNEUMA_IFACE="$(ip -4 route show default | awk '{print $5; exit}')"
PNEUMA_CIDR="$(ip -4 -o addr show dev "$PNEUMA_IFACE" | awk '{print $4; exit}')"
PNEUMA_GW="$(ip -4 route show default | awk '{print $3; exit}')"
# Take the UPSTREAM resolvers, not /etc/resolv.conf -- on a systemd-resolved
# host that file holds the 127.0.0.53 stub, and writing the stub into netplan
# points resolution at itself. IPv6 resolvers are filtered out deliberately:
# this interface has no IPv6 route, so an AAAA-only resolver would just time out.
PNEUMA_DNS="$(awk '/^nameserver/ {print $2}' /run/systemd/resolve/resolv.conf 2>/dev/null \
  | grep -v ':' | grep -v '^127\.' | paste -sd, -)"
if [ -z "$PNEUMA_DNS" ]; then
  PNEUMA_DNS="$(resolvectl dns 2>/dev/null | tr ' ' '\n' | grep -E '^[0-9]+\.' | grep -v '^127\.' | sort -u | paste -sd, -)"
fi
if [ -n "$PNEUMA_IFACE" ] && [ -n "$PNEUMA_CIDR" ] && [ -n "$PNEUMA_GW" ]; then
  sudo tee /etc/netplan/99-pneuma-ipv4-only.yaml >/dev/null <<NETPLAN
# Pneuma: eth0 is IPv4-only ON PURPOSE. The host's IPv6 egress to ghcr.io is
# dead (connection reset), and an IPv6 address here makes containerd prefer a
# path that cannot complete a pull. Generated by setup.sh from the live
# interface -- re-run setup.sh after any re-addressing.
network:
  version: 2
  ethernets:
    ${PNEUMA_IFACE}:
      ...
      accept-ra: false
      link-local: [ipv4]
      addresses: ["${PNEUMA_CIDR}"]
      nameservers:
        addresses: [${PNEUMA_DNS}]
      routes:
        - to: default
          via: ${PNEUMA_GW}
NETPLAN
  ...
  echo "WARN: could not derive IPv4 config for netplan; leaving cloud-init netplan in place" >&2
fi
